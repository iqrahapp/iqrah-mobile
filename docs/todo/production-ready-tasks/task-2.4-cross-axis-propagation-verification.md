...
- **Example:** `VERSE:1:1:translation` → `VERSE:1:1:memorization` with weight 0.3
...
async fn test_tafsir_helps_translation() {
    ...
    let initial_energy = get_node_energy(&user_repo, "VERSE:1:1:translation").await;

    // Review tafsir node
    learning_service
        .record_review("default", "VERSE:1:1:tafsir", Rating::Good)
        .await
        .unwrap();

    let after_energy = get_node_energy(&user_repo, "VERSE:1:1:translation").await;
...
async fn test_propagation_event_recorded() {
    ...
    let events = user_repo
        .get_propagation_events("default", "VERSE:1:1:translation", 10)
        .await
        .unwrap();
    ...
    let cross_axis_event = events.iter().find(|e| {
        e.target_node_id == "VERSE:1:1:memorization"
    });
...
async fn test_multiple_axes_accumulate_energy() {
    ...
    let initial_energy = get_node_energy(&user_repo, "VERSE:1:1:memorization").await;
    ...
    let after_energy = get_node_energy(&user_repo, "VERSE:1:1:memorization").await;
...
cargo run --bin iqrah-cli -- stats --node-id "VERSE:1:10:memorization"

echo ""
echo "=== Scenario 1: Translation → Memorization ==="
cargo run --bin iqrah-cli -- review --node-id "VERSE:1:10:translation" --rating good
cargo run --bin iqrah-cli -- stats --node-id "VERSE:1:10:memorization"

echo ""
echo "=== Scenario 2: Tafsir → Translation → Memorization ==="
cargo run --bin iqrah-cli -- review --node-id "VERSE:1:11:tafsir" --rating good
cargo run --bin iqrah-cli -- stats --node-id "VERSE:1:11:translation"
cargo run --bin iqrah-cli -- stats --node-id "VERSE:1:11:memorization"

echo ""
echo "=== Scenario 3: Direct Memorization ==="
cargo run --bin iqrah-cli -- review --node-id "VERSE:1:12:memorization" --rating good
cargo run --bin iqrah-cli -- stats --node-id "VERSE:1:13:memorization"
...
- Review VERSE:1:10:memorization → Next verse energy: X.XX

**Scenario 2: Translation First**
- Review VERSE:1:11:translation → memorization energy: X.XX
- Then review VERSE:1:11:memorization → Next verse energy: X.XX + Y.YY

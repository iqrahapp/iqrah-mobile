use iqrah_storage::content::init_test_content_db;
...
async fn test_content_db_has_version() {
    ...
    let pool = init_test_content_db(db_path.to_str().unwrap())
        .await
        .unwrap();
...
async fn test_user_db_has_version() {
    ...
    assert_eq!(version, "2.0.0", "User DB should be version 2.0.0");
}

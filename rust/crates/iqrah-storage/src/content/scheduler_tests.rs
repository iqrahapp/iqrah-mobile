...
async fn test_get_scheduler_candidates_with_metadata() {
    ...
    // Insert test data - using TEXT node_ids to match migration schema
...
async fn test_get_scheduler_candidates_missing_metadata_defaults_to_zero() {
    ...
    // Insert node without metadata - using TEXT node_id to match migration schema
...
async fn test_get_prerequisite_parents_single_node() {
    ...
    // Using TEXT node_ids to match migration schema
    query("INSERT INTO nodes (ukey, node_type) VALUES ('1:1', 'verse')")
        .execute(&pool)
        .await
        .unwrap();
...
async fn test_get_prerequisite_parents_multiple_parents() {
    ...
    // Create multiple prerequisites for 1:3 - using TEXT node_ids to match migration schema
    query("INSERT INTO nodes (ukey, node_type) VALUES ('1:1', 'verse')")
        .execute(&pool)
        .await
        .unwrap();

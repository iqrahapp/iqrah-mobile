...
DEFAULT_UTHMANI_FILE = "uthmani-simple.json"
...
def build_audio_url(
    ...
    """Build the audio URL for a given word location.
...
def mp3_bytes_to_pcm_s16le(mp3_data: bytes, sample_rate: int = 16000, channels: int = 1) -> bytes:
...
async def worker(
    ...
    Also maintains a text→audio map for later patching of missing words.

    Audio is stored as Ogg Opus (.ogg) on disk, but hashes are computed on the
    original MP3 bytes from the source.
    """
    while True:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Download QuranWBW word audio, deduplicate by hash, and map locations to unique audio files.",
    )
    p.add_argument(
        "--uthmani",
        type=str,

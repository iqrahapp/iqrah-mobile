...
**Example: T1.1.1 - Audio Format Support**

```markdown
## Task: Implement Multi-Format Audio Loader

**Module**: M1 (Preprocessing)
**Location**: `src/iqrah_audio/preprocessing/audio_loader.py`

**Requirements**:
1. Support: MP3, WAV, WebM, M4A, FLAC
2. Validate: Check file format header, detect corruption
3. Extract: Duration, sample rate, bit depth, channels
4. Return: Standardized dict format
5. Error handling: Raise AudioLoadError with clear message

**Input**: `file_path: str`
**Output**:
```python
{
    "audio": np.ndarray,    # (n_samples,) mono
    "sample_rate": int,
    "duration": float,
    "metadata": {
        "original_sr": int,
...
**Test Cases**:
1. Valid WAV → Return correct dict
2. Corrupted file → Raise AudioLoadError
3. Unsupported format → Raise AudioLoadError
4. Stereo → Convert to mono (average channels)
...
### Template 2: Training Task

**Example: T3.1.2 - Continue Pretraining**

```markdown
## Task: Continue Pretrain Wav2Vec2-BERT on Quranic Audio

**Module**: M3 (Alignment)
**Location**: `scripts/train_wav2vec2_continue.py`

**Requirements**:
1. Download: `facebook/w2v-bert-2.0` from HuggingFace
2. Load: Tarteel dataset via `datasets` library
...
**Deliverables**:
- `models/wav2vec2_bert_quranic/` checkpoint
- Training logs (loss curve, WER validation)
- `README.md` with hyperparameters

**Validation**:
- Validation loss decreases vs base model
...
**Example: T4.2.1 - Ghunnah Data Collection**

```markdown
## Task: Collect Ghunnah Training Examples

**Module**: M4 (Tajweed)
**Location**: `scripts/prepare_ghunnah_dataset.py`

**Goal**: 1000+ labeled examples (ghunnah present/absent)

**Steps**:
1. Extract all noon saakinah, meem, tanween from existing phoneme data
2. Use Tajweed mapping to identify ghunnah cases
3. Sample negative examples (non-ghunnah nasals)
4. Export CSV: audio_path, start, end, has_ghunnah
5. Verify extractability of all snippets
...
**Example: T7.1.1 - Validator Aggregation**

```markdown
## Task: Aggregate Tajweed Validators in Comparison Engine

**Module**: M7 (Comparison)
**Location**: `src/iqrah_audio/comparison/engine.py`

**Requirements**:
1. Call: MaddDurationValidator, GhunnahValidator, QalqalahValidator
...
5. Return: `{overall, madd, ghunnah, qalqalah, violations}`
...
- Validator exception → log warning, skip validator
- Zero phonemes for rule → score = N/A (not 0)

**Test Cases**:
- Perfect recitation → overall = 100
- 1 madd violation of 5 → madd score = 80
- Validator crash → graceful degradation
...
| Stream        | Tasks         | Start      | Duration |
| ------------- | ------------- | ---------- | -------- |
| Voice Quality | T5.1-T5.5     | Week 1     | 2 weeks  |
| Prosody       | T6.1-T6.3     | After M5   | 3 weeks  |
| Feedback      | T8.1-T8.4     | After M7   | 1 week   |
| Ghunnah       | T4.2.1-T4.2.5 | After data | 2 weeks  |
| Qalqalah      | T4.3.1-T4.3.5 | After data | 3 weeks  |
...
| Weeks | Focus                   | Output              |
| ----- | ----------------------- | ------------------- |
| 1-4   | M3 alignment            | Phoneme recognition |
| 5-8   | M4 tajweed              | Basic rules         |
| 9-12  | M6-M7 prosody+compare   | Full pipeline       |
| 13-16 | M8+V1 feedback+validate | Prototype           |
| 17-20 | M4.3 qalqalah+refine    | Advanced rules      |
| 21-24 | User test+iterate       | Production          |

### With AI Agents (2-3× Faster)

| Week  | Main          | Agent 1     | Agent 2        | Agent 3       |
| ----- | ------------- | ----------- | -------------- | ------------- |
| 1-2   | M3.1 train    | M1 preproc  | M2 pitch       | M5 OpenSMILE  |
| 3-4   | M3.2 align    | M4.1 madd   | M8.1 templates | V1.1.1 data   |
| 5-6   | M4.2 ghunnah  | M6.1 rhythm | M7.1 compare   | T5.5 embed    |
| 7-8   | M4.3 qalqalah | M6.2 melody | M7.5 fusion    | T8.3 progress |
| 9-10  | Integration   | Docs        | Viz            | Benchmark     |
| 11-12 | User test     | Bug fix     | Refine         | Deploy        |
...
**Main Developer**:
- Setup Wav2Vec2 training pipeline
- Monitor training curves

**Agent 1**: T1.1.1 - Implement audio loader
**Agent 2**: T2.1.1 - Integrate SwiftF0
**Agent 3**: T5.1.1 - Setup OpenSMILE extraction

**By End of Sprint**: All data preprocessing done, training started

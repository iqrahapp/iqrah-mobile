...
try:
    from src.iqrah_audio.analysis.pitch_extractor_swiftf0 import extract_pitch_swiftf0, SWIFT_F0_AVAILABLE
except ImportError:
    SWIFT_F0_AVAILABLE = False
    extract_pitch_swiftf0 = None
...
async def analyze_qari(surah: int, ayah: int, pitch_extractor: str = "swiftf0"):
    ...
    try:
        ...
        else:  # swiftf0 (default) - fallback to crepe if not available
            if SWIFT_F0_AVAILABLE:
                pitch_data = extract_pitch_swiftf0(audio_path)
            else:
                print("   ⚠️  SwiftF0 not available, using CREPE-fast instead...")
                pitch_data = extract_pitch_crepe_fast(audio_path)
...
if __name__ == "__main__":
    ...
    if not SWIFT_F0_AVAILABLE:
        ...
        print("   Using CREPE as fallback (slower but accurate)")
        print("   To install: pip install swift-f0")
